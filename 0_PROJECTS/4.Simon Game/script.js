...
function checkAnswer(currentLevel) {
  if (gamePattern[currentLevel] === userClickedPattern[currentLevel]) {
    ...
    if (userClickedPattern.length === gamePattern.length) {
      ...
      setTimeout(function() {    //call nextSequence afer user finished his sequence
        nextSequence();
      }, 1000);
    }
  } else {
    ...
    setTimeout(function() { //Remove game-over class
...
function nextSequence() {  //Function for next sequence
  userClickedPattern = [];
...
function animatePress(currentColour) {  //Function to animate button click
...
function playSound(name) {    //Audio play
  var audio = new Audio("sounds/" + name + ".mp3");
  audio.play();
}

function startOver() {  //Restart function
  level = 0;
  gamePattern = [];
  started = false;
}
